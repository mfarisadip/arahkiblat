...
# Fungsi Lintang Posisi Kita

LB = -6.902469 # Lintang dalam Desimal
Cos_LB = cos(radians(LB)) # Cos Lintang Posisi
Sin_LB = sin(radians(LB)) # Sin Lintang Posisi

# Bujur Posisi Kita
BB = 106.930283 # Bujur dalam Desimal
Cos_BB = cos(radians(BB)) # Cos Bujur Posisi
Sin_BB = sin(radians(BB)) # Sin Bujur Posisi
...
# Menentukan Selisih Bujur Lokasi Kita dan Kota Mekkah
BBA = (BB - BA)
...
if LB<LA and BB>BA:
    ...
    S_B1 = round((M_B1 - int(M_B1))*60.0)
    B2 = int(D_B1), int(M_B1), int(S_B1)
    print("B (Kiblat Terhadap Utara) = ", B2)
